...
1Panel 是一款现代化的服务器管理面板，支持 Docker 容器编排和可视化管理。

**步骤 1：安装 1Panel**
参考官方文档：https://1panel.cn/docs/quickstart/install.html

**步骤 2：进入 1Panel 后台，打开“应用商店”或“容器管理”**

**步骤 3：新建容器编排任务**
1. 选择“Docker Compose”编排。
2. 上传或粘贴本项目的 `docker-compose.yml` 文件内容。
3. 一键启动服务。

**步骤 4：访问服务**
在 1Panel 的端口映射中确认 3000 端口已开放，浏览器访问 `http://服务器IP:3000`。

**常见问题**
- 如需自定义环境变量，可在编排界面直接编辑 compose 文件。
- 日志、重启、停止等操作均可在 1Panel 容器管理界面完成。

---

### 4.2 通过宝塔面板部署

宝塔面板支持 Docker 管理插件，可用于快速部署本项目。

**步骤 1：安装宝塔面板及 Docker 管理插件**
参考官方文档：https://www.bt.cn/new/download.html

**步骤 2：进入宝塔后台，打开“Docker管理器”插件**

**步骤 3：导入镜像并创建容器**
1. 在“镜像管理”中构建或拉取 easy-cover 镜像（可用 `docker build` 或 Docker Hub 镜像）。
2. 在“容器管理”中新建容器，设置端口映射（如 3000:80），并选择刚才构建的镜像。
3. 启动容器。

**步骤 4：访问服务**
在宝塔面板端口安全设置中开放 3000 端口，浏览器访问 `http://服务器IP:3000`。

**常见问题**
- 可在宝塔面板中管理容器的启动、停止、重启、日志查看等。
- 如需批量更新，建议使用 compose 文件或脚本自动化。
